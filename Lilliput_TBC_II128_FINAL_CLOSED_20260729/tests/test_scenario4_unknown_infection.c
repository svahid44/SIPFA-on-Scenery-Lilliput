...
#define DATASET_SEED UINT64_C(0x510E527FADE682D1)
...
int main(void)
{
    ...
    /*
     * Attack boundary: only the unlabeled public ciphertexts and their count
     * are supplied.  SECRET_DELTA, key, tweak, event labels, and actual_rtk are
     * deliberately absent from the recovery interface.
     */
    ...
    if (status != 0) {
        fprintf(stderr,
                "FAIL: unknown-infection attack returned %d\n",
                status);
        free(published_ciphertexts);
        return 1;
    }
    ...
    if (memcmp(result.recovered_round_tweakey,
        ...
        free(published_ciphertexts);
        return fail("unknown-infection recovery did not reproduce RTK[31]");
    }

    if (result.candidate_rank[SECRET_DELTA] != 1U) {
        free(published_ciphertexts);
        return fail("correct delta is not ranked first by aggregate SEI");
    }

    if ((result.best_candidate_count != 1U) ||
        (result.best_score <= result.second_best_score)) {
        free(published_ciphertexts);
        return fail("SEI ranking does not have a unique positive gap");
    }
    ...
    for (size_t lane = 0; lane < LILLIPUT_LAST_ROUND_LANES; ++lane) {
        ...
        for (size_t value = 0; value < LILLIPUT_SBOX_DOMAIN; ++value) {
            total += result.final_histogram[lane][value];
        }

        if (total != (uint64_t)SAMPLE_COUNT) {
            free(published_ciphertexts);
            return fail("a final histogram does not contain every public output");
        }
        if (result.final_minimum_multiplicity[lane] != 1U) {
            free(published_ciphertexts);
            return fail("a final-round lane has a tied minimum");
        }
        if (result.final_minimum[lane] != expected_minimum) {
            fprintf(stderr,
        ...
        if (result.final_lane_sei[lane] <= 0.0) {
            free(published_ciphertexts);
            return fail("a final-round SEI is not positive");
        }
    }
    ...
    lilliput_fault_reset();
    free(published_ciphertexts);
    puts("PASS: Scenario 4 unknown-fault infection-based SEI recovery verified.");
    return 0;
}
