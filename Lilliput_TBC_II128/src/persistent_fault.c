#include <stdint.h>
#include <string.h>

#include "persistent_fault.h"

static const uint8_t S_CORRECT[256] = {
    
};

static uint8_t S_FAULTY[256];
static int initialized = 0;
static int active = 0;
static uint8_t active_input = 0;
static uint8_t active_output = 0;

static void ensure_initialized(void)
{
    if (!initialized) {
        memcpy(S_FAULTY, S_CORRECT, sizeof(S_FAULTY));
        initialized = 1;
    }
}

void lilliput_fault_reset(void)
{
    memcpy(S_FAULTY, S_CORRECT, sizeof(S_FAULTY));
    initialized = 1;
    active = 0;
    active_input = 0;
    active_output = 0;
}

int lilliput_fault_inject(uint8_t input, uint8_t faulty_output)
{
    ensure_initialized();

    if (faulty_output == S_CORRECT[input]) {
        return -1;
    }

    /* The model contains exactly one faulty entry at a time. */
    memcpy(S_FAULTY, S_CORRECT, sizeof(S_FAULTY));
    S_FAULTY[input] = faulty_output;
    active = 1;
    active_input = input;
    active_output = faulty_output;
    return 0;
}

int lilliput_fault_is_active(void)
{
    return active;
}

uint8_t lilliput_fault_input(void)
{
    return active_input;
}

uint8_t lilliput_fault_output(void)
{
    return active_output;
}

uint8_t lilliput_fault_correct_output(void)
{
    return S_CORRECT[active_input];
}

uint8_t lilliput_sbox_correct(uint8_t input)
{
    return S_CORRECT[input];
}

uint8_t lilliput_sbox_faulty(uint8_t input)
{
    ensure_initialized();
    return S_FAULTY[input];
}
